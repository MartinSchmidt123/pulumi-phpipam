...
First, clone this repo with the name of the desired provider in place of `phpipam`:

```
git clone https://github.com/pulumi/pulumi-tf-provider-boilerplate pulumi-phpipam
```

Second, replace references to `phpipam` with the name of your provider:
...
To use from Go, use `go get` to grab the latest version of the library
    ...
    $ go get github.com/pulumi/pulumi-phpipam/sdk/go/...

## Configuration

The following configuration points are available for the `phpipam` provider:

- `phpipam:apiKey` (environment: `XYZ_API_KEY`) - the API key for `phpipam`
- `phpipam:region` (environment: `XYZ_REGION`) - the region in which to deploy resources
